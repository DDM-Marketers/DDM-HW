...
The calibration sample was selected using oversampling, however the validation sample was not created using oversampling and represents the true churn rate in the date. This section we compared and contrasted the churn Rate in the training sample and validation sample. 
...
print(pe)
```
The churn rate in the training sample is 49.8%, close to 50%, but the true churn rate in the validation sample is much lower, at 1.93%. This verifies that approximately 50% of observations in the training sample take the value, contrasted with the true churn rate in the valiation sample. 
\newpage
...
The following independent variables are significant with 95% confidence. 
